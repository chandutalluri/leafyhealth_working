/**
 * Enhanced API Hooks for identity-access
...
const API_PREFIX = getApiPrefix('identity-access');
...
export function useIdentityAccess() {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<IdentityAccessItem>) => {
      return await apiClient.post(API_PREFIX, newItem);
    },
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: ['identity-access'] });
      setItems(prev => [...prev, newItem]);
      toast({
        title: 'Success',
        description: 'identity access item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<IdentityAccessItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'identity access item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'identity access item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
