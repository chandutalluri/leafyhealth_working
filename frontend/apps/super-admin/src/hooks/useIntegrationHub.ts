/**
 * Enhanced API Hooks for integration-hub
...
const API_PREFIX = getApiPrefix('integration-hub');

interface IntegrationHubItem {
  id: string;
  [key: string]: any;
}
...
export function useIntegrationHub() {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<IntegrationHubItem>) => {
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'integration hub item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<IntegrationHubItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'integration hub item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'integration hub item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
