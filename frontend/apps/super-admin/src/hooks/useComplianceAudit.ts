/**
 * Enhanced API Hooks for compliance-audit
...
const API_PREFIX = getApiPrefix('compliance-audit');

interface ComplianceAuditItem {
  id: string;
  [key: string]: any;
}

export function useComplianceAudit() {
  const [items, setItems] = useState<ComplianceAuditItem[]>([]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'compliance audit item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'compliance audit item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
