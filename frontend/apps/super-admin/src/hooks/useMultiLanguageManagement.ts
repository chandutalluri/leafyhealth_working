/**
 * Enhanced API Hooks for multi-language-management
...
const API_PREFIX = getApiPrefix('multi-language-management');

interface MultiLanguageManagementItem {
  id: string;
  [key: string]: any;
}

export function useMultiLanguageManagement() {
  const [items, setItems] = useState<MultiLanguageManagementItem[]>([]);
  ...
  const {
    ...
    queryKey: ['multi-language-management', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<MultiLanguageManagementItem>) => {
      return await apiClient.post(API_PREFIX, newItem);
    },
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: ['multi-language-management'] });
      setItems(prev => [...prev, newItem]);
      toast({
        title: 'Success',
        description: 'multi language management item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<MultiLanguageManagementItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'multi language management item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'multi language management item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
