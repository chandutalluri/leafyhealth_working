/**
 * Enhanced API Hooks for performance-monitor
...
const API_PREFIX = getApiPrefix('performance-monitor');

interface PerformanceMonitorItem {
  id: string;
  [key: string]: any;
}

export function usePerformanceMonitor() {
  const [items, setItems] = useState<PerformanceMonitorItem[]>([]);
  ...
  const {
    ...
    queryKey: ['performance-monitor', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<PerformanceMonitorItem>) => {
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'performance monitor item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<PerformanceMonitorItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'performance monitor item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'performance monitor item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
