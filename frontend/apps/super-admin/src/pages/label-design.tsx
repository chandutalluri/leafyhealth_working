...
export default function LabelDesignPage() {
  const [data, setData] = useState<LabelDesignData[]>([])
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch from actual label-design microservice
      const apiGateway = process.env.NEXT_PUBLIC_API_GATEWAY || 'http://localhost:8080';
      const response = await fetch(`${apiGateway}/api/label-design`)
      
      if (!response.ok) {
        throw new Error('Failed to fetch label-design data')
      }
      ...
      console.error('Error fetching label-design data:', err)
      setError('Failed to connect to label-design service')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <SuperAdminLayout title="Label Design" subtitle="Manage label design operations">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white shadow-lg rounded-xl border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-900">Label Design</h2>
                ...
                {data.length > 0 ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-8 text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                      The label design service is connected but no data is available yet.
                    </p>
                    <button 
                      onClick={fetchData}
