...
interface AnalyticsReportingData {
  id: number
  name: string
  status: string
  createdAt: string
...
export default function AnalyticsReportingPage() {
  const [data, setData] = useState<AnalyticsReportingData[]>([])
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch from actual analytics-reporting microservice
      const apiGateway = process.env.NEXT_PUBLIC_API_GATEWAY || 'http://localhost:8080';
      const response = await fetch(`${apiGateway}/api/analytics-reporting`)
      
      if (!response.ok) {
        throw new Error('Failed to fetch analytics-reporting data')
      }
      ...
      setError('Failed to connect to analytics-reporting service')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <SuperAdminLayout title="Analytics Reporting" subtitle="Manage analytics reporting operations">
                ...
                {data.length > 0 ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-8 text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                      The analytics reporting service is connected but no data is available yet.
                    </p>
                    <button 
                      onClick={fetchData}
