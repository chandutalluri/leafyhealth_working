...
export default function PaymentProcessingPage() {
  const [data, setData] = useState<PaymentProcessingData[]>([])
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch from actual payment-processing microservice
      const apiGateway = process.env.NEXT_PUBLIC_API_GATEWAY || 'http://localhost:8080';
      const response = await fetch(`${apiGateway}/api/payment-processing`)
      
      if (!response.ok) {
        throw new Error('Failed to fetch payment-processing data')
      }
      ...
      console.error('Error fetching payment-processing data:', err)
      setError('Failed to connect to payment-processing service')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <SuperAdminLayout title="Payment Processing" subtitle="Manage payment processing operations">
                ...
                {data.length > 0 ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-8 text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                      The payment processing service is connected but no data is available yet.
                    </p>
                    <button 
                      onClick={fetchData}
