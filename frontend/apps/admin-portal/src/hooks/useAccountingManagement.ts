/**
 * Enhanced API Hooks for accounting-management
...
const getApiPrefix = (domain: string): string => {
...
const API_PREFIX = getApiPrefix('accounting-management');
...
export function useAccountingManagement() {
  ...
  const {
    ...
    queryKey: ['accounting-management', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<AccountingManagementItem>) => {
      return await apiClient.post(API_PREFIX, newItem);
    },
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: ['accounting-management'] });
      setItems(prev => [...prev, newItem]);
      toast({
        title: 'Success',
        description: 'accounting management item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<AccountingManagementItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'accounting management item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'accounting management item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
