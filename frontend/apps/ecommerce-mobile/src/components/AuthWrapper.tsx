  ...
  const checkAuthentication = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Check if user is authorized for Mobile Delivery app
        const allowedRoles = ['super-admin', 'delivery']
        ...
        // Check if user's assigned app matches current app (unless super admin)
        const userAssignedApp = user.assignedApp
        const currentAppMatches = userAssignedApp === 'delivery'
        
        if (!currentAppMatches && user.role !== 'super-admin') {
          // Redirect to correct app
          const assignedAppUrl = getAppUrl(userAssignedApp)
          window.location.href = assignedAppUrl
          return
        }
