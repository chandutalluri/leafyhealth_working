/**
 * Enhanced API Hooks for shipping-delivery
...
const API_PREFIX = getApiPrefix('shipping-delivery');
...
export function useShippingDelivery() {
  ...
  const {
    ...
    queryKey: ['shipping-delivery', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<ShippingDeliveryItem>) => {
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'shipping delivery item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<ShippingDeliveryItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'shipping delivery item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'shipping delivery item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
