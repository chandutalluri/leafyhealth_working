/**
 * Enhanced API Hooks for employee-management
...
const API_PREFIX = getApiPrefix('employee-management');
...
export function useEmployeeManagement() {
  ...
  const {
    ...
    queryKey: ['employee-management', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<EmployeeManagementItem>) => {
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'employee management item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<EmployeeManagementItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'employee management item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'employee management item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
