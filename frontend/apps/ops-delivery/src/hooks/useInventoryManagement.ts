/**
 * Enhanced API Hooks for inventory-management
...
const API_PREFIX = getApiPrefix('inventory-management');

interface InventoryManagementItem {
  id: string;
  [key: string]: any;
}

export function useInventoryManagement() {
  const [items, setItems] = useState<InventoryManagementItem[]>([]);
  ...
  const {
    ...
    queryKey: ['inventory-management', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<InventoryManagementItem>) => {
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'inventory management item created successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<InventoryManagementItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'inventory management item updated successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'inventory management item deleted successfully',
      });
    },
    onError: (error: any) => {
      if (error instanceof APIError && error.status === 401) {
