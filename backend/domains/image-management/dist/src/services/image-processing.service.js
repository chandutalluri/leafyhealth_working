...
let ImageProcessingService = ImageProcessingService_1 = class ImageProcessingService {
    constructor() {
        ...
        this.baseUrl = process.env.BASE_URL || 'http://localhost:3070';
        this.variants = {
    ...
    async processImage(tempFilePath, originalName, options = {}) {
        ...
        try {
            ...
            for (const variantKey of variantsToGenerate) {
                if (!generateThumbnails && variantKey.includes('sm'))
                    continue;
    ...
    async deleteImage(imagePath, variants = []) {
        try {
    ...
    async optimizeImage(filePath, options = {}) {
        const { quality = 85, format = 'jpeg', width, height } = options;
